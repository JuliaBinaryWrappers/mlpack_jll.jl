...
if VERSION < v"1.3.0-rc4"
    ...
    error("Unable to import mlpack_jll on Julia versions older than 1.3!")
end
    ...
    function dev_jll()
        ...
        mktempdir() do temp_env
            Pkg.activate(temp_env) do
                Pkg.develop("mlpack_jll")
            end
        end
        # Create the override directory
        override_dir = joinpath(Pkg.devdir(), "mlpack_jll", "override")
        # Copy the current artifact contents into that directory
        if !isdir(override_dir)
            cp(artifact"mlpack", override_dir)
        end
        # Force recompilation of that package, just in case it wasn't dev'ed before
        touch(joinpath(Pkg.devdir(), "mlpack_jll", "src", "mlpack_jll.jl"))
        @info("mlpack_ll dev'ed out to /depot/dev/mlpack_jll with pre-populated override directory")
    end
end
...
artifacts = Pkg.Artifacts.load_artifacts_toml(artifacts_toml; pkg_uuid=UUID("156a7db3-27ba-586e-a86b-f061da4f95ea"))
platforms = [Pkg.Artifacts.unpack_platform(e, "mlpack", artifacts_toml) for e in artifacts["mlpack"]]
...
if best_platform === nothing
    @debug("Unable to load mlpack; unsupported platform $(triplet(platform_key_abi()))")
else
