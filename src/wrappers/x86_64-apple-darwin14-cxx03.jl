# Autogenerated wrapper script for mlpack_jll for x86_64-apple-darwin14-cxx03
export libmlpack_julia_preprocess_binarize, libmlpack_julia_hmm_loglik, libmlpack_julia_perceptron, libmlpack_julia_hmm_generate, libmlpack_julia_hmm_train, libmlpack_julia_kfn, libmlpack_julia_adaboost, libmlpack_julia_logistic_regression, libmlpack_julia_krann, libmlpack_julia_preprocess_scale, libmlpack_julia_lars, libmlpack_julia_gmm_train, libmlpack_julia_util, libmlpack_julia_hoeffding_tree, libmlpack_julia_kmeans, libmlpack, libmlpack_julia_fastmks, libmlpack_julia_det, libmlpack_julia_gmm_generate, libmlpack_julia_decision_tree, libmlpack_julia_approx_kfn, libmlpack_julia_random_forest, libmlpack_julia_mean_shift, libmlpack_julia_knn, libmlpack_julia_decision_stump, libmlpack_julia_radical, libmlpack_julia_cf, libmlpack_julia_sparse_coding, libmlpack_julia_local_coordinate_coding, libmlpack_julia_linear_svm, libmlpack_julia_lsh, libmlpack_julia_nbc, libmlpack_julia_gmm_probability, libmlpack_julia_image_converter, libmlpack_julia_kernel_pca, libmlpack_julia_softmax_regression, libmlpack_julia_nmf, libmlpack_julia_preprocess_describe, libmlpack_julia_pca, libmlpack_julia_emst, libmlpack_julia_preprocess_split, libmlpack_julia_hmm_viterbi, libmlpack_julia_dbscan, libmlpack_julia_lmnn, libmlpack_julia_nca, libmlpack_julia_linear_regression
...
const libmlpack_julia_preprocess_binarize_splitpath = ["lib", "libmlpack_julia_preprocess_binarize.dylib"]
...
const libmlpack_julia_preprocess_binarize = "@rpath/libmlpack_julia_preprocess_binarize.dylib"


# Relative path to `libmlpack_julia_hmm_loglik`
const libmlpack_julia_hmm_loglik_splitpath = ["lib", "libmlpack_julia_hmm_loglik.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_hmm_loglik = "@rpath/libmlpack_julia_hmm_loglik.dylib"


# Relative path to `libmlpack_julia_perceptron`
const libmlpack_julia_perceptron_splitpath = ["lib", "libmlpack_julia_perceptron.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_perceptron = "@rpath/libmlpack_julia_perceptron.dylib"


# Relative path to `libmlpack_julia_hmm_generate`
const libmlpack_julia_hmm_generate_splitpath = ["lib", "libmlpack_julia_hmm_generate.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_hmm_generate = "@rpath/libmlpack_julia_hmm_generate.dylib"


# Relative path to `libmlpack_julia_hmm_train`
const libmlpack_julia_hmm_train_splitpath = ["lib", "libmlpack_julia_hmm_train.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_hmm_train = "@rpath/libmlpack_julia_hmm_train.dylib"


# Relative path to `libmlpack_julia_kfn`
const libmlpack_julia_kfn_splitpath = ["lib", "libmlpack_julia_kfn.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_kfn = "@rpath/libmlpack_julia_kfn.dylib"


# Relative path to `libmlpack_julia_adaboost`
const libmlpack_julia_adaboost_splitpath = ["lib", "libmlpack_julia_adaboost.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_adaboost = "@rpath/libmlpack_julia_adaboost.dylib"


# Relative path to `libmlpack_julia_logistic_regression`
const libmlpack_julia_logistic_regression_splitpath = ["lib", "libmlpack_julia_logistic_regression.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_logistic_regression = "@rpath/libmlpack_julia_logistic_regression.dylib"


# Relative path to `libmlpack_julia_krann`
const libmlpack_julia_krann_splitpath = ["lib", "libmlpack_julia_krann.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_krann = "@rpath/libmlpack_julia_krann.dylib"


# Relative path to `libmlpack_julia_preprocess_scale`
const libmlpack_julia_preprocess_scale_splitpath = ["lib", "libmlpack_julia_preprocess_scale.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_preprocess_scale = "@rpath/libmlpack_julia_preprocess_scale.dylib"


# Relative path to `libmlpack_julia_lars`
const libmlpack_julia_lars_splitpath = ["lib", "libmlpack_julia_lars.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_lars = "@rpath/libmlpack_julia_lars.dylib"


# Relative path to `libmlpack_julia_gmm_train`
const libmlpack_julia_gmm_train_splitpath = ["lib", "libmlpack_julia_gmm_train.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_gmm_train = "@rpath/libmlpack_julia_gmm_train.dylib"


# Relative path to `libmlpack_julia_util`
const libmlpack_julia_util_splitpath = ["lib", "libmlpack_julia_util.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_util = "@rpath/libmlpack_julia_util.dylib"


# Relative path to `libmlpack_julia_hoeffding_tree`
const libmlpack_julia_hoeffding_tree_splitpath = ["lib", "libmlpack_julia_hoeffding_tree.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_hoeffding_tree = "@rpath/libmlpack_julia_hoeffding_tree.dylib"


# Relative path to `libmlpack_julia_kmeans`
const libmlpack_julia_kmeans_splitpath = ["lib", "libmlpack_julia_kmeans.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_kmeans = "@rpath/libmlpack_julia_kmeans.dylib"


# Relative path to `libmlpack`
const libmlpack_splitpath = ["lib", "libmlpack.3.3.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack = "@rpath/libmlpack.3.dylib"


# Relative path to `libmlpack_julia_fastmks`
const libmlpack_julia_fastmks_splitpath = ["lib", "libmlpack_julia_fastmks.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_fastmks = "@rpath/libmlpack_julia_fastmks.dylib"


# Relative path to `libmlpack_julia_det`
const libmlpack_julia_det_splitpath = ["lib", "libmlpack_julia_det.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_det = "@rpath/libmlpack_julia_det.dylib"


# Relative path to `libmlpack_julia_gmm_generate`
const libmlpack_julia_gmm_generate_splitpath = ["lib", "libmlpack_julia_gmm_generate.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_gmm_generate = "@rpath/libmlpack_julia_gmm_generate.dylib"


# Relative path to `libmlpack_julia_decision_tree`
const libmlpack_julia_decision_tree_splitpath = ["lib", "libmlpack_julia_decision_tree.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_decision_tree = "@rpath/libmlpack_julia_decision_tree.dylib"


# Relative path to `libmlpack_julia_approx_kfn`
const libmlpack_julia_approx_kfn_splitpath = ["lib", "libmlpack_julia_approx_kfn.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_approx_kfn = "@rpath/libmlpack_julia_approx_kfn.dylib"


# Relative path to `libmlpack_julia_random_forest`
const libmlpack_julia_random_forest_splitpath = ["lib", "libmlpack_julia_random_forest.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_random_forest = "@rpath/libmlpack_julia_random_forest.dylib"


# Relative path to `libmlpack_julia_mean_shift`
const libmlpack_julia_mean_shift_splitpath = ["lib", "libmlpack_julia_mean_shift.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_mean_shift = "@rpath/libmlpack_julia_mean_shift.dylib"


# Relative path to `libmlpack_julia_knn`
const libmlpack_julia_knn_splitpath = ["lib", "libmlpack_julia_knn.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_knn = "@rpath/libmlpack_julia_knn.dylib"


# Relative path to `libmlpack_julia_decision_stump`
const libmlpack_julia_decision_stump_splitpath = ["lib", "libmlpack_julia_decision_stump.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_decision_stump = "@rpath/libmlpack_julia_decision_stump.dylib"


# Relative path to `libmlpack_julia_radical`
const libmlpack_julia_radical_splitpath = ["lib", "libmlpack_julia_radical.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_radical = "@rpath/libmlpack_julia_radical.dylib"


# Relative path to `libmlpack_julia_cf`
const libmlpack_julia_cf_splitpath = ["lib", "libmlpack_julia_cf.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_cf = "@rpath/libmlpack_julia_cf.dylib"


# Relative path to `libmlpack_julia_sparse_coding`
const libmlpack_julia_sparse_coding_splitpath = ["lib", "libmlpack_julia_sparse_coding.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_sparse_coding = "@rpath/libmlpack_julia_sparse_coding.dylib"


# Relative path to `libmlpack_julia_local_coordinate_coding`
const libmlpack_julia_local_coordinate_coding_splitpath = ["lib", "libmlpack_julia_local_coordinate_coding.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_local_coordinate_coding = "@rpath/libmlpack_julia_local_coordinate_coding.dylib"


# Relative path to `libmlpack_julia_linear_svm`
const libmlpack_julia_linear_svm_splitpath = ["lib", "libmlpack_julia_linear_svm.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_linear_svm = "@rpath/libmlpack_julia_linear_svm.dylib"


# Relative path to `libmlpack_julia_lsh`
const libmlpack_julia_lsh_splitpath = ["lib", "libmlpack_julia_lsh.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_lsh = "@rpath/libmlpack_julia_lsh.dylib"


# Relative path to `libmlpack_julia_nbc`
const libmlpack_julia_nbc_splitpath = ["lib", "libmlpack_julia_nbc.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_nbc = "@rpath/libmlpack_julia_nbc.dylib"


# Relative path to `libmlpack_julia_gmm_probability`
const libmlpack_julia_gmm_probability_splitpath = ["lib", "libmlpack_julia_gmm_probability.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_gmm_probability = "@rpath/libmlpack_julia_gmm_probability.dylib"


# Relative path to `libmlpack_julia_image_converter`
const libmlpack_julia_image_converter_splitpath = ["lib", "libmlpack_julia_image_converter.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_image_converter = "@rpath/libmlpack_julia_image_converter.dylib"


# Relative path to `libmlpack_julia_kernel_pca`
const libmlpack_julia_kernel_pca_splitpath = ["lib", "libmlpack_julia_kernel_pca.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_kernel_pca = "@rpath/libmlpack_julia_kernel_pca.dylib"


# Relative path to `libmlpack_julia_softmax_regression`
const libmlpack_julia_softmax_regression_splitpath = ["lib", "libmlpack_julia_softmax_regression.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_softmax_regression = "@rpath/libmlpack_julia_softmax_regression.dylib"


# Relative path to `libmlpack_julia_nmf`
const libmlpack_julia_nmf_splitpath = ["lib", "libmlpack_julia_nmf.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_nmf = "@rpath/libmlpack_julia_nmf.dylib"


# Relative path to `libmlpack_julia_preprocess_describe`
const libmlpack_julia_preprocess_describe_splitpath = ["lib", "libmlpack_julia_preprocess_describe.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_preprocess_describe = "@rpath/libmlpack_julia_preprocess_describe.dylib"


# Relative path to `libmlpack_julia_pca`
const libmlpack_julia_pca_splitpath = ["lib", "libmlpack_julia_pca.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_pca = "@rpath/libmlpack_julia_pca.dylib"


# Relative path to `libmlpack_julia_emst`
const libmlpack_julia_emst_splitpath = ["lib", "libmlpack_julia_emst.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_emst = "@rpath/libmlpack_julia_emst.dylib"


# Relative path to `libmlpack_julia_preprocess_split`
const libmlpack_julia_preprocess_split_splitpath = ["lib", "libmlpack_julia_preprocess_split.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_preprocess_split = "@rpath/libmlpack_julia_preprocess_split.dylib"


# Relative path to `libmlpack_julia_hmm_viterbi`
const libmlpack_julia_hmm_viterbi_splitpath = ["lib", "libmlpack_julia_hmm_viterbi.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_hmm_viterbi = "@rpath/libmlpack_julia_hmm_viterbi.dylib"


# Relative path to `libmlpack_julia_dbscan`
const libmlpack_julia_dbscan_splitpath = ["lib", "libmlpack_julia_dbscan.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_dbscan = "@rpath/libmlpack_julia_dbscan.dylib"


# Relative path to `libmlpack_julia_lmnn`
const libmlpack_julia_lmnn_splitpath = ["lib", "libmlpack_julia_lmnn.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_lmnn = "@rpath/libmlpack_julia_lmnn.dylib"


# Relative path to `libmlpack_julia_nca`
const libmlpack_julia_nca_splitpath = ["lib", "libmlpack_julia_nca.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_nca = "@rpath/libmlpack_julia_nca.dylib"


# Relative path to `libmlpack_julia_linear_regression`
const libmlpack_julia_linear_regression_splitpath = ["lib", "libmlpack_julia_linear_regression.dylib"]
...
# This must be `const` so that we can use it with `ccall()`
const libmlpack_julia_linear_regression = "@rpath/libmlpack_julia_linear_regression.dylib"
...
function __init__()
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libmlpack_julia_preprocess_scale_handle = dlopen(libmlpack_julia_preprocess_scale_path)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libmlpack_julia_linear_svm_handle = dlopen(libmlpack_julia_linear_svm_path)
    ...
    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libmlpack_julia_image_converter_handle = dlopen(libmlpack_julia_image_converter_path)
