...
export libmlpack_julia_kfn, libmlpack_julia_util, libmlpack_julia_gmm_generate, libmlpack_julia_gmm_train, libmlpack_julia_lars, libmlpack_julia_mean_shift, libmlpack_gmm_probability, libmlpack_julia_det, libmlpack_julia_hoeffding_tree, libmlpack_julia_nbc, libmlpack_julia_pca, libmlpack_julia_fastmks, libmlpack_julia_krann, libmlpack_julia_sparse_coding, libmlpack_julia_decision_tree, libmlpack_julia_hmm_viterbi, libmlpack_julia_random_forest, libmlpack_julia_kmeans, libmlpack_julia_nmf, libmlpack_julia_radical, libmlpack_julia_decision_stump, libmlpack_julia_lmnn, libmlpack_julia_hmm_loglik, libmlpack_julia_lsh, libmlpack_julia_adaboost, libmlpack_julia_cf, libmlpack_julia_preprocess_describe, libmlpack_julia_local_coordinate_coding, libmlpack_julia_emst, libmlpack_julia_knn, libmlpack_julia_kernel_pca, libmlpack_julia_linear_regression, libmlpack_julia_hmm_train, libmlpack_julia_approx_kfn, libmlpack_julia_nca, libmlpack_julia_softmax_regression, libmlpack, libmlpack_julia_preprocess_binarize, libmlpack_julia_hmm_generate, libmlpack_julia_preprocess_split, libmlpack_julia_dbscan, libmlpack_julia_perceptron
...
# Relative path to `libmlpack_gmm_probability`
const libmlpack_gmm_probability_splitpath = ["lib", "libmlpack_julia_gmm_probability.dylib"]

# This will be filled out by __init__() for all products, as it must be done at runtime
libmlpack_gmm_probability_path = ""

# libmlpack_gmm_probability-specific global declaration
# This will be filled out by __init__()
libmlpack_gmm_probability_handle = C_NULL

# This must be `const` so that we can use it with `ccall()`
const libmlpack_gmm_probability = "@rpath/libmlpack_julia_gmm_probability.dylib"
...
const libmlpack_splitpath = ["lib", "libmlpack.3.2.dylib"]
...
function __init__()
    ...
    global libmlpack_gmm_probability_path = normpath(joinpath(artifact_dir, libmlpack_gmm_probability_splitpath...))

    # Manually `dlopen()` this right now so that future invocations
    # of `ccall` with its `SONAME` will find this path immediately.
    global libmlpack_gmm_probability_handle = dlopen(libmlpack_gmm_probability_path)
    push!(LIBPATH_list, dirname(libmlpack_gmm_probability_path))
